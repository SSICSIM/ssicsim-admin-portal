  ...
  async function doQueue() {
    ...
    try {
      ...
      if (activeTemplate?.confirms_assigned && assignedInAudience.length > 0) {
        promotions.push({
          delegates: assignedInAudience,
          status: "Confirmed",
          label: "marked as Confirmed"
        });
      }
      if (activeTemplate?.confirms_payment && verifyPaymentInAudience.length > 0) {
        promotions.push({
          delegates: verifyPaymentInAudience,
          status: "Awaiting Assignment",
          label: "moved to Awaiting Assignment"
        });
      }
        ...
        try {
          await Promise.all(
            promotions.flatMap(({ delegates: ds, status }) =>
              ds.map((d) =>
                updateDelegate.mutateAsync({ delegateId: d.id, data: { delegate_status: status } })
              )
            )
          );
          setConfirmStatusMsg(
            promotions
              .map(
                ({ delegates: ds, label }) =>
                  `${ds.length} delegate${ds.length !== 1 ? "s" : ""} ${label}.`
              )
              .join(" ")
          );
        } catch {
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isActive
                    ? "bg-[var(--ssicsim-brand-gold-soft)] text-[var(--ssicsim-brand-navy)]"
                    : isComplete
              ...
              >
                <span
                  className={[
                    ...
                    isActive
                      ? "border-[var(--ssicsim-brand-gold)] bg-[var(--ssicsim-brand-gold)] text-white"
                      : isComplete
                ...
                <div className="grid gap-5 sm:grid-cols-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setDelegationFilter(v as UUID | "all")}
                    >
                      <SelectTrigger>
                ...
                {selectedDelegates.length > 0 ? (
                  <div className="rounded-xl border border-[var(--ssicsim-border)] bg-[var(--ssicsim-surface-soft)] p-4">
                    <p className="text-sm font-semibold text-[var(--ssicsim-brand-navy)]">
                      {selectedDelegates.length} recipient
                      {selectedDelegates.length !== 1 ? "s" : ""} selected
                    </p>
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {selectedDelegates.slice(0, 14).map((d) => (
                        <span
                          key={d.id}
                          className="rounded-full border border-[var(--ssicsim-border)] bg-white px-2.5 py-0.5 text-xs text-[var(--ssicsim-text)]"
                        >
                  ...
                  {templatesQuery.isLoading ? (
                    ...
                    <div className="rounded-xl border border-[var(--ssicsim-border)] bg-[var(--ssicsim-surface-soft)] px-4 py-3 text-sm text-[var(--ssicsim-text-muted)]">
                      ...
                      <Link
                        href="/emailer/templates"
                        target="_blank"
                        className="font-medium text-[var(--ssicsim-brand-gold)] hover:underline"
                      >
                        Create one ↗
                      </Link>
                    </div>
              ...
              <>
                ...
                <div className="rounded-xl border border-[var(--ssicsim-border)] bg-[var(--ssicsim-surface-soft)] p-4 space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    <p className="font-semibold text-[var(--ssicsim-brand-navy)]">
                      {selectedDelegates.length} email{selectedDelegates.length !== 1 ? "s" : ""} to
                      send
                    </p>
                    {activeTemplate && (
                ...
                {activeTemplate?.confirms_assigned && assignedInAudience.length > 0 && (
                  <div className="rounded-xl border border-[var(--ssicsim-brand-gold)]/40 bg-[var(--ssicsim-brand-gold-soft)] px-4 py-3">
                    ...
                    <p className="mt-0.5 text-sm text-[var(--ssicsim-text-muted)]">
                      After sending,{" "}
                      <strong className="text-[var(--ssicsim-brand-navy)]">
                        {assignedInAudience.length} Assigned delegate
                        {assignedInAudience.length !== 1 ? "s" : ""}
                      </strong>{" "}
                      will be automatically moved to{" "}
                      <strong className="text-[var(--ssicsim-brand-navy)]">Confirmed</strong>.
                    </p>
                  </div>
                )}
                ...
                {activeTemplate?.confirms_payment && verifyPaymentInAudience.length > 0 && (
                  <div className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3">
                    ...
                    <p className="mt-0.5 text-sm text-[var(--ssicsim-text-muted)]">
                      After sending,{" "}
                      <strong className="text-[var(--ssicsim-brand-navy)]">
                        {verifyPaymentInAudience.length} Verify Payment delegate
                        {verifyPaymentInAudience.length !== 1 ? "s" : ""}
                      </strong>{" "}
                      will be automatically moved to{" "}
                      <strong className="text-[var(--ssicsim-brand-navy)]">
                        Awaiting Assignment
                  ...
                  <div className="max-h-56 overflow-y-auto rounded-xl border border-[var(--ssicsim-border)]">
                    {selectedDelegates.map((d) => (
                      ...
                      >
                        ...
                        <span className="text-[var(--ssicsim-text-muted)] truncate max-w-[200px]">
                          {d.email}
                        </span>
                      </div>
                    ))}
                ...
                {results && (
                  <div className="space-y-1.5">
                    ...
                    {confirmingStatus && (
                      <p className="text-xs text-[var(--ssicsim-text-muted)]">
                        Updating delegate statuses…
                      </p>
                    )}
                    {confirmStatusMsg && (
                      <p className="text-xs font-medium text-[var(--ssicsim-brand-navy)]">
                        {confirmStatusMsg}
                      </p>
                    )}
                  </div>
          ...
          {awaitingConfirm && validationWarnings && validationWarnings.length > 0 && (
            <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-3">
              <p className="text-sm font-semibold text-amber-800">
                {validationWarnings.length} address{validationWarnings.length !== 1 ? "es" : ""}{" "}
                flagged — emails may not be delivered
              </p>
              ...
              <p className="text-xs text-amber-600">
                These will still be passed to the worker. Invalid addresses will fail at the SMTP
                level.
              </p>
              <div className="flex gap-2">
                <Button size="sm" onClick={doQueue} disabled={sending}>
