...
class Interview(Base):
    ...
    sec_member_id1: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("sec_member.id", ondelete="SET NULL")
    )
    sec_member_id2: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("sec_member.id", ondelete="SET NULL")
    )
    
    isConfirmed1: Mapped[bool | None] = mapped_column(Boolean)
    isConfirmed2: Mapped[bool | None] = mapped_column(Boolean)
